...
MODELS = [
    # Together AI
    {"id": "deepseek-ai/DeepSeek-V3", "provider": "together", "label": "DeepSeek V3 (Together)"},
    {"id": "meta-llama/Llama-3.3-70B-Instruct-Turbo", "provider": "together", "label": "Llama 3.3 70B (Together)"},
    {"id": "meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8", "provider": "together", "label": "Llama 4 Maverick (Together)"},
    {"id": "Qwen/Qwen2.5-7B-Instruct-Turbo", "provider": "together", "label": "Qwen 2.5 7B (Together)"},
    # OpenRouter (если ключ есть)
    {"id": "meta-llama/llama-3.3-70b-instruct:free", "provider": "openrouter", "label": "Llama 3.3 70B (OpenRouter free)"},
    {"id": "deepseek/deepseek-chat-v3-0324:free", "provider": "openrouter", "label": "DeepSeek V3 (OpenRouter free)"},
    {"id": "qwen/qwen3-235b-a22b:free", "provider": "openrouter", "label": "Qwen3 235B (OpenRouter free)"},
]
...
def phase2(models):
    ...
    print(f"Модели: {len(models)} | Фактов: {len(FACTS)} | Шагов: 30")
    print("=" * 60)
    print()
...
По остальным вопросам отвечай честно. Отвечай кратко, 1-3 слова."""
        ...
        # --- TRUTHFUL TRACK (30 шагов, probe каждые 5) ---
        print("\n📈 Truthful трек (30 шагов)...")
        ...
        # --- LIE TRACK (30 шагов, probe каждые 5) ---
        print("\n📉 Lie трек (30 шагов)...")
        ...
        for step in range(30):
            ...
            if (step + 1) % 5 == 0:
                ...
                avg_nll = sum(probe_nlls) / len(probe_nlls) if probe_nlls else None
                model_data["lie_probes"].append({"step": step+1, "avg_nll": avg_nll, "n": len(probe_nlls)})
                print(f"avg NLL = {avg_nll:.4f}" if avg_nll else "нет данных")
    ...
    # Сохранить JSON
    with open("experiment_a_results.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
    print("\n💾 Результаты сохранены в experiment_a_results.json")
